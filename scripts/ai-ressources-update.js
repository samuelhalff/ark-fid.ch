...
function buildSystemPrompt(frJson, trendData = null, topicAnalysis = null) {
  ...
  if (FORCE_TOPIC) {
    ...
    if (topicAnalysis) {
      ...
      const recentTopicAnalysis = analyzeRecentTopics(frJson, 15);
      const avoidTopicsLabels = recentTopicAnalysis.avoidTopics.map(describeTopic);
      const suggestedTopics = recentTopicAnalysis.underrepresented.slice(0, 4);
      if (avoidTopicsLabels.length > 0) {
        diversityGuidance.push(
          `⚠️ THÈMES À ÉVITER (traités récemment dans les ${recentTopicAnalysis.rotationWindow} derniers articles): ${avoidTopicsLabels.join(", ")}.`,
        );
      }
      ...
      if (recentTopicAnalysis.overrepresented.length > 0) {
        const overLabels = recentTopicAnalysis.overrepresented.map(
  ...
  if (!FORCE_TOPIC) {
    diversityGuidance.push(
      "Priorité éditoriale: élargis activement la couverture vers les thèmes conformité LBA/AML, domiciliation, incorporation/registre du commerce, gouvernance corporate et mobilité internationale lorsqu'ils sont disponibles.",
    );
  }
...
function buildResearchPrompt(frJson, trendData, seoSuggestions, topicAnalysis = null) {
  const today = isoDateToday();
  const twelveMonthsAgo = (() => {
    const d = new Date();
    d.setMonth(d.getMonth() - 12);
  ...
  } else if (topicAnalysis) {
    diversityGuidance.push(buildTopicGuidanceForAgent(topicAnalysis));
  } else {
    if (avoidTopicsLabels.length > 0) {
      diversityGuidance.push(
        `⚠️ THÈMES À ÉVITER (sur les ${recentTopicAnalysis.rotationWindow} derniers articles): ${avoidTopicsLabels.join(", ")}.`,
      );
    }
    if (suggestedTopics.length > 0) {
      diversityGuidance.push(
  ...
  if (!FORCE_TOPIC) {
    diversityGuidance.push(
      "Priorité éditoriale: fais remonter des angles pratiques sur AML/LBA, bénéficiaires effectifs, domiciliation, registre du commerce, structures d'incorporation, gouvernance et obligations locales quand ces thèmes sont disponibles.",
    );
  }
...
function validateResearchPayload(frData, payload, topicAnalysis = null) {
  ...
  if (topicAnalysis) {
    const topicCheck = validateArticleTopicAgainstAvailable(
      research,
      topicAnalysis,
    );
    if (!topicCheck.valid) {
      const err = new Error(topicCheck.error);
      err.code = "TOPIC_NOT_IN_AVAILABLE_POOL";
      err.topic = topicCheck.detectedTopic;
      throw err;
    }
  }
...
async function generateResearchWithRetries(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    try {
      ...
      const research = validateResearchPayload(frData, payload, topicAnalysis);
